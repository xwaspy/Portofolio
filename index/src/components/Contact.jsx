...
const Contact = () => {
  ...
  const sendEmail = (e) => {
    ...
    emailjs.sendForm('Portofolio_Email', 'template_nuqq9wr', form.current, 'cZAxokqCmFBNFdwxT')
      .then((result) => {
  ...
  return (
    <div id="Contact" className="flex min-h-screen w-full 
    flex-col items-center justify-center gap-16 p-8">
      <h1 className="text-center text-6xl font-light text-purple-600">Get in Touch</h1>

      <form ref={form} onSubmit={sendEmail} className="flex w-full max-w-md flex-col gap-8 
      rounded-lg p-6 md:max-w-lg lg:max-w-xl">
        ...
        <textarea placeholder="Your Message" name="message" required className="h-32 w-full resize-none 
        rounded-lg border-2 border-purple-400 px-4 py-3 text-lg outline-none 
        transition-all duration-200 hover:bg-purple-100 focus:ring-purple-500"></textarea>
        <button type="submit" className="rounded-lg border-2 border-purple-400\
         bg-purple-500 px-6 py-3 font-semibold text-white
         transition-all duration-200 hover:bg-purple-600">Send Message</button>
      </form>
